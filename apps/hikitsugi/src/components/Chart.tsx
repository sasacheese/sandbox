 ...
 * 上がっていく区間は藍（代理）で、そこから先の短い区間だけが緑（あなた）。
...
export function Chart({
  ...
  return (
    <figure className="chart">
      <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="近さの変化。代理同士がやり取りした期間で立ち上がり、引き継いだあとがあなたの区間">
        ...
        {people.map((person) => {
          ...
          return (
            <g key={person.id}>
              {/* 代理がやり取りした区間。少し撓ませて、人の手で積まれたようには見せない */}
              <path
                d={`M ${start} ${y(0)} Q ${(start + peak) / 2} ${y(height * 0.45)} ${peak} ${y(height)}`}
                className="chart__proxyLine"
              />
              <circle cx={start} cy={y(0)} r={2.4} className="chart__proxyDot" />
      ...
      <figcaption className="chart__legend">
        <span className="chart__key chart__key--proxy">代理がやり取りした</span>
